namespace DAL.GETMigrations
{
    ...
    public partial class insert_get_implement_equipment1 : DbMigration
    {
...
begin
	if exists (select impserial from GET where Upper(impserial)=Upper(@impserial))
		...
		begin
			...
			set @sqlValues = 'VALUES ('''+@impserial+''','+cast(@implement_auto as varchar)+', 1,'+cast(@equipmentid_auto as varchar)+','+cast(@installsmu as varchar)+' ,'+cast(@created_user as varchar)+' , '''+cast(@installdate AS varchar)+''', '+cast(@make_auto as varchar)+' , ' + cast(@num as varchar) + ' '
