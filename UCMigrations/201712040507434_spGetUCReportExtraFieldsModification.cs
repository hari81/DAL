namespace DAL.UCMigrations
{
    ...
    public partial class spGetUCReportExtraFieldsModification : DbMigration
    {
        public override void Up()
        {
            Sql(spGetUCReportExtraFields);
        }
        ...
        string spGetUCReportExtraFields = @"
SET ANSI_NULLS ON
GO
...
BEGIN
	...
	isnull((SELECT TOP 1 file_data FROM (select Top 2 * from EQ_UNIT_ATTACHMENTS_FILESTREAM  Where equipmentid_auto=e.equipmentid_auto ORDER BY equnit_attachment_auto DESC) x ORDER BY equnit_attachment_auto),(select UCReportImg2 from track_ucreport_defaultimages)) as img2, cu.customer_auto
	FROM TRACK_QUOTE tq
		INNER JOIN TRACK_INSPECTION ti ON tq.inspection_auto = ti.inspection_auto
		INNER JOIN EQUIPMENT e ON e.equipmentid_auto = ti.equipmentid_auto
...
BEGIN
	...
	ISNULL((SELECT filedata FROM BLOB_FAMILYIMAGE WHERE blob_auto = t.blob_large_auto),(SELECT top(1) UCReportImg2 FROM track_ucreport_defaultimages)) as img2, 
	ISNULL((SELECT filedata FROM BLOB_FAMILYIMAGE WHERE blob_auto = t.blob_large_auto),(SELECT top(1) UCReportImg2 FROM track_ucreport_defaultimages)) as img3,
	cu.customer_auto
	FROM TRACK_QUOTE tq
		INNER JOIN TRACK_INSPECTION ti ON tq.inspection_auto = ti.inspection_auto
		INNER JOIN EQUIPMENT e ON e.equipmentid_auto = ti.equipmentid_auto
...
BEGIN
	...
	--,ISNULL(dei.overall_evaluation,'X') AS [OverallEval]
	,isnull(ti.evalcode,(select max(eval_code) from track_inspection_detail where inspection_auto = ti.inspection_auto)) as OverallEval
	,ISNULL((SELECT filedata FROM BLOB_FAMILYIMAGE_LARGE WHERE blob_auto = t.blob_large_auto),(SELECT top(1) UCReportImg1 FROM track_ucreport_defaultimages)) as img1,
	ISNULL((SELECT filedata FROM BLOB_FAMILYIMAGE_LARGE WHERE blob_auto = t.blob_large_auto),(SELECT top(1) UCReportImg2 FROM track_ucreport_defaultimages)) as img2, cu.customer_auto
	FROM TRACK_QUOTE tq
		INNER JOIN TRACK_INSPECTION ti ON tq.inspection_auto = ti.inspection_auto
		INNER JOIN EQUIPMENT e ON e.equipmentid_auto = ti.equipmentid_auto
		INNER JOIN CRSF crsf ON crsf.crsf_auto = e.crsf_auto
