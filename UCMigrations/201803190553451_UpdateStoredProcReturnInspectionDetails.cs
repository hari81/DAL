namespace DAL.UCMigrations
{
    ...
    public partial class UpdateStoredProcReturnInspectionDetails : DbMigration
    {
        ...
        string sql1 = @"--=============================================================
-- Created By:	NB | 09-06-16 | PRN10920
--=============================================================
ALTER PROCEDURE [dbo].[spReturnTrackInspectionWithUCSubSystem]
	@inspection_auto INT,
 ...
 AS
	...
	IF (@inspection_auto!=0)
        ...
        string sql2 = @"/*-----------------------------------------------------------------------------------
-- Change			NB | 09-06-2016 | PRN10920 -- Added UC Chain and UC Frame 
-- Change:			NB | 03-03-2016 | PRN9692
-- Changed By:		Nouman Bhatti
-- Changed Date:	14-08-2015
-- Reason:			Changed the queries to cater the history tables records - PRN9082
-- Test Run:		exec return_track_inspection_4T 0,332306
--------------------------------------------------------------------------------------
*/
ALTER PROCEDURE [dbo].[return_track_inspection_4T]
	@inspection_auto INT,
	@equipmentid_auto BIGINT
 AS
	...
	SELECT @value = value_key FROM APPLICATION_LU_CONFIG WHERE variable_key = 'UCSubSysSerialShow'
	
	IF @value = '1'
	BEGIN
		EXEC spReturnTrackInspectionWithUCSubSystem @inspection_auto,@equipmentid_auto
	END
	ELSE
	BEGIN
		IF (@inspection_auto!=0)
	BEGIN
		...
		SELECT lct.comparttype + ' : ' + lu.compart as compart, lu.compartid_auto, lu.track_comp_cts_maintype, lu.track_comp_cts_subtype,
				...
				(SELECT COUNT(*) 
					...
					WHERE compartid_auto = lu.compartid_auto AND inspection_auto = @inspection_auto AND compart_attach_type_auto = 4 AND (position IS NULL OR position = pos)) AS imgCountRight,
					tid.inspection_detail_auto ,lu.compartid
					,lct.sorder, lu.compartid_auto, geu.pos,geu.side,lu.smcs_code,0 AS historytable_auto,geu.cmu,lct.comparttypeid, lct.comparttype_auto, lct.sorder --added comparttypeid
		--FROM  GENERAL_EQ_UNIT AS geu  
		--		INNER JOIN	EQUIPMENT AS e ON e.equipmentid_auto = geu.equipmentid_auto 
		--		INNER JOIN	LU_COMPART lu ON geu.compartid_auto=lu.compartid_auto 
		--		INNER JOIN	LU_COMPART_TYPE lct ON lu.comparttype_auto = lct.comparttype_auto
		...
		SELECT lct.comparttype + ' : ' + lu.compart as compart, lu.compartid_auto, lu.track_comp_cts_maintype, lu.track_comp_cts_subtype,
				...
				(SELECT COUNT(*) 
					...
					WHERE compartid_auto = lu.compartid_auto AND inspection_auto = @inspection_auto AND compart_attach_type_auto = 4 AND (position IS NULL OR position = pos)) AS imgCountRight,
					tid.inspection_detail_auto ,lu.compartid
					,lct.sorder, lu.compartid_auto, geu.pos,geu.side,lu.smcs_code, geu.equnit_auto AS historytable_auto,geu.cmu,lct.comparttypeid, lct.comparttype_auto, lct.sorder --added comparttypeid
		FROM  GENERAL_EQ_UNIT_HISTORY AS geu  
				left JOIN	EQUIPMENT AS e ON e.equipmentid_auto = geu.equipmentid_auto 
				INNER JOIN	LU_COMPART lu ON geu.compartid_auto=lu.compartid_auto 
				INNER JOIN	LU_COMPART_TYPE lct ON lu.comparttype_auto = lct.comparttype_auto
	...
	BEGIN
		SELECT lct.comparttype + ' : ' + lu.compart as compart, lu.compartid_auto, lu.track_comp_cts_maintype, lu.track_comp_cts_subtype,
			...
			'' AS ext_remaining_hours,'' AS expected_life, '' AS ext_expected_life,'' AS eval_code,geu.equnit_auto,lu.smcs_code,geu.smu_at_install, tt.tool_auto,'' AS comments, 
			geu.track_0_worn, geu.track_100_worn, geu.track_120_worn, geu.eq_ltd_at_install, geu.smu_at_install, 0 AS imgCountLeft, 0 AS imgCountRight, '' AS inspection_detail_auto 
			,lu.compartid,geu.equipmentid_auto, 0 AS historytable_auto,geu.cmu,lct.comparttypeid, lct.comparttype_auto, lct.sorder --added comparttypeid
